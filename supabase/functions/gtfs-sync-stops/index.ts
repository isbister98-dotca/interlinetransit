  ...
  try {
    ...
    for (const feed of feeds || []) {
      ...
      await supabase.from("gtfs_sync_status").upsert({
        agency_id: agencyId, file_type: "stops", status: "running",
      ...
      try {
        ...
        // stops.txt
        const stopsKey = Object.keys(files).find(k => k.endsWith("stops.txt"));
        if (stopsKey) {
          const text = new TextDecoder().decode(files[stopsKey]);
          const rows = parseCSV(text);
          await supabase.from("gtfs_stops").delete().eq("agency_id", agencyId);
          const mapped = rows.map(r => ({
            agency_id: agencyId,
            stop_id: r.stop_id,
            stop_name: r.stop_name || null,
            stop_lat: r.stop_lat ? parseFloat(r.stop_lat) : null,
            stop_lon: r.stop_lon ? parseFloat(r.stop_lon) : null,
            zone_id: r.zone_id || null,
            stop_url: r.stop_url || null,
            location_type: r.location_type ? parseInt(r.location_type) : null,
            parent_station: r.parent_station || null,
            wheelchair_boarding: r.wheelchair_boarding ? parseInt(r.wheelchair_boarding) : null,
            stop_code: r.stop_code || null,
          }));
          await batchInsert(supabase, "gtfs_stops", mapped);
          totalRows += mapped.length;
        }

        await supabase.from("gtfs_sync_status").upsert({
          agency_id: agencyId, file_type: "stops", status: "done",
          row_count: totalRows, completed_at: new Date().toISOString(), error_msg: null,
        }, { onConflict: "agency_id,file_type" });
        results[agencyId] = { ok: true, rows: totalRows };
      } catch (e) {
        await supabase.from("gtfs_sync_status").upsert({
          agency_id: agencyId, file_type: "stops", status: "error",
